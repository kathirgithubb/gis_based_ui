// BLACK_GREY — RESPONSIVE v6 (smaller mobile menu popup + smaller OC text + mobile-safe OC width/offset)
...
export default function GISUiResponsiveV4() {
  ...
  useEffect(() => {
    ...
    poles.forEach((pole) => {
      const m = L.circleMarker([pole.lat, pole.lon], {
        radius: 5,
        color: '#555',
        weight: 2,
        fillColor: '#111',
  ...
  }, [showOC, selectedObjectId, showBottomPanel, isMobile]);

  const getPoleValue = (pole: Pole, field: string) =>
    pole.attributes.find((a) => a.field === field)?.value || '-';
  ...
  const zoomToSelected = () => {
    const p = getSelectedPole();
    if (p) leafletMapRef.current?.setView([p.lat, p.lon], 18);
  };
  ...
  const makeMH = (id: string, status: string, designId: string, extra?: Partial<BottomRow>): BottomRow => ({
    key: id,
    id,
    type: 'Manhole',
    status,
  ...
  const makeCB = (id: string, status: string, designId: string, extra?: Partial<BottomRow>): BottomRow => ({
    key: id,
    id,
    type: 'Cabinate',
    status,
  ...
  const bottomPanelRows: BottomRow[] =
    selectedObjectItem === 'Pole'
      ? poles.map((pole) => ({
          key: pole.id,
          id: pole.id,
          type: getPoleValue(pole, 'feature_type'),
          status: getPoleValue(pole, 'status'),
          owner: getPoleValue(pole, 'owner'),
          material: getPoleValue(pole, 'material'),
          height: getPoleValue(pole, 'height_m'),
          municipality: getPoleValue(pole, 'municipality'),
          designId: getPoleValue(pole, 'design_id'),
          selected: selectedObjectId === pole.id,
          onClick: () => openPole(pole),
        }))
      : selectedObjectItem === 'Manhole'
      ? [
          makeMH('MH-001', 'Active', 'MH-2101', {
            selected: selectedObjectId === 'MH-001',
            onClick: () => createObjectData('Manhole'),
          }),
          makeMH('MH-002', 'Proposed', 'MH-2102', {
            onClick: () => {
              setSelectedObjectItem('Manhole');
              setSelectedObjectId('MH-002');
              const d: Attribute[] = [
                { field: 'asset_id', value: 'MH-002' },
                { field: 'feature_type', value: 'Manhole' },
                { field: 'status', value: 'Proposed' },
                { field: 'owner', value: 'Utility Network' },
                { field: 'material', value: 'Concrete' },
                { field: 'municipality', value: 'Quezon City' },
                { field: 'design_id', value: 'MH-2102' },
              ];
              setAttributes(d);
              setDraftAttributes(d);
              setIsEditing(false);
              setShowSaveMenu(false);
            },
          }),
      ...
      ? [
          makeCB('CB-001', 'Active', 'CB-3101', {
            selected: selectedObjectId === 'CB-001',
            onClick: () => createObjectData('Cabinate'),
          }),
          makeCB('CB-002', 'Inactive', 'CB-3102', {
            onClick: () => {
              setSelectedObjectItem('Cabinate');
              setSelectedObjectId('CB-002');
              const d: Attribute[] = [
                { field: 'asset_id', value: 'CB-002' },
                { field: 'feature_type', value: 'Cabinate' },
                { field: 'status', value: 'Inactive' },
                { field: 'owner', value: 'Metro Utility' },
                { field: 'material', value: 'Steel' },
                { field: 'municipality', value: 'Quezon City' },
                { field: 'design_id', value: 'CB-3102' },
              ];
              setAttributes(d);
              setDraftAttributes(d);
              setIsEditing(false);
  ...
  const filteredRows = useMemo(() => {
    ...
    return tableFilterMode === 'By ID'
      ? bottomPanelRows.filter((r) => r.id.toLowerCase().includes(q))
      : bottomPanelRows;
  ...
  /* layout helpers */
  const ocPanelWidth = showOC ? (isMobile ? Math.min(window.innerWidth * 0.3, 140) : 250) : 0;
  const topLeftLeft = showOC ? ocPanelWidth + 12 : 12;
  const botLeftLeft = showOC ? ocPanelWidth + 12 : 12;
  ...
  const ib =
    'flex items-center justify-center border border-[#c8c8c8] bg-[#e8e8e8] text-[#111] shadow-sm transition-all hover:bg-[#d4d4d4] active:bg-[#c0c0c0]';
    ...
    return (
      ...
      >
        ...
        {Array.from({ length: 16 }, (_, i) => {
          ...
          return (
            <line
              ...
              x2={27 + r2 * Math.sin(rad)}
              y2={27 - r2 * Math.cos(rad)}
        ...
        {[45, 135, 225, 315].map((deg) => {
          ...
          const l = { x: 27 + 3.2 * Math.sin(rad + Math.PI / 2), y: 27 - 3.2 * Math.cos(rad + Math.PI / 2) };
          const r = { x: 27 + 3.2 * Math.sin(rad - Math.PI / 2), y: 27 - 3.2 * Math.cos(rad - Math.PI / 2) };
          return <polygon key={deg} points={`${tip.x},${tip.y} ${l.x},${l.y} ${r.x},${r.y}`} fill="#c0c0c0" />;
        })}
        ...
        <text x="27" y="15" textAnchor="middle" fontSize={5.5 * scale > 3 ? 5.5 : 5} fontWeight="700" fill="#111" fontFamily="sans-serif">
          N
        </text>
        <text x="27" y="44.5" textAnchor="middle" fontSize={5 * scale > 3 ? 5 : 4.6} fontWeight="500" fill="#999" fontFamily="sans-serif">
          S
        </text>
        <text x="43.5" y="28.8" textAnchor="middle" fontSize={5 * scale > 3 ? 5 : 4.6} fontWeight="500" fill="#999" fontFamily="sans-serif">
          E
        </text>
        <text x="10.5" y="28.8" textAnchor="middle" fontSize={5 * scale > 3 ? 5 : 4.6} fontWeight="500" fill="#999" fontFamily="sans-serif">
          W
        </text>
      </svg>
    );
  ...
  const renderDetails = (small: boolean) =>
    (isEditing ? draftAttributes : attributes).map((item, idx) => (
      <div
        key={item.field}
        className={`grid items-center border-b border-[#d4d4d4] leading-none
        ${small ? 'min-h-[24px] grid-cols-[82px_1fr] text-[11px]' : 'min-h-[26px] grid-cols-[100px_1fr] text-[12px]'}
        ${idx % 2 === 0 ? 'bg-[#e8e8e8]' : 'bg-[#efefef]'}`}
      >
          ...
          {isEditing ? (
            item.field === 'status' ? (
              <select
                ...
                className={`w-full rounded border border-[#c8c8c8] bg-white px-1.5 text-[#111] outline-none ${
  ...
  const renderLayers = (small: boolean) =>
    ['Pole', 'Substation', 'Cabinate', 'Cable'].map((layer, idx) => (
      <div
        key={layer}
        className={`grid grid-cols-[1fr_auto] items-center border-b border-[#d4d4d4] px-3 leading-none
    ...
    <div className="relative h-screen w-full overflow-hidden bg-[#f2f2f2] font-sans text-[#111]">
      ...
      <div className="absolute top-2 z-30 flex items-center gap-1.5 transition-all duration-300" style={{ left: `${topLeftLeft}px` }}>
        ...
        <div className="flex h-8 sm:h-9 w-[160px] sm:w-[240px] items-center rounded-full border border-[#c8c8c8] bg-[#f2f2f2]/98 px-3 shadow-sm">
          <input
            className="w-full bg-transparent text-[11px] sm:text-[12px] text-[#111] outline-none placeholder:text-[#888]"
            placeholder="Search..."
          />
        </div>
            ...
            {[
              {
                label: 'Home',
                action: () => {
                  leafletMapRef.current?.setView([14.5943, 121.1866], 13);
                  setShowTopMenu(false);
                },
              },
              ...
              <button
                ...
                type="button"
                onClick={item.action}
                className={`flex w-full items-center text-left text-[#111] hover:bg-[#d4d4d4] ${
                  isMobile ? 'px-3 py-2 text-[11px]' : 'px-4 py-2.5 text-[13px]'
                } ${i > 0 ? 'border-t border-[#c8c8c8]' : ''}`}
              >
                {item.label}
              </button>
            ))}
      ...
      <div className={`absolute top-2 z-30 flex items-center gap-1.5 sm:gap-2 transition-all duration-300 ${rightShift}`}>
        ...
        <button type="button" title="User" className={`${ib} h-8 w-8 sm:h-9 sm:w-9 rounded-full`}>
          <span className="text-sm">👤</span>
        </button>
      </div>

      {/* RIGHT COLUMN: basemap + tools */}
      <div className={`absolute top-[46px] sm:top-[52px] z-30 flex flex-col gap-1.5 transition-all duration-300 ${rightShift}`}>
        <div className="relative">
          ...
          {showBaseMapDropdown && (
            <div className="absolute right-0 top-10 sm:top-11 z-50 min-w-[150px] overflow-hidden rounded-xl border border-[#c8c8c8] bg-[#e8e8e8] shadow-lg">
              <div className="border-b border-[#c8c8c8] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[#555]">
                Base Map
              </div>
              {baseMaps.map((map, i) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between px-3 py-2 text-left text-[12px] transition-all
                    ${selectedBaseMap === map ? 'bg-[#111] text-white' : 'text-[#111] hover:bg-[#d4d4d4]'} ${
                    i > 0 ? 'border-t border-[#c8c8c8]' : ''
                  }`}
                >
                  <span>{map}</span>
                  {selectedBaseMap === map && <span className="text-xs">✓</span>}
                </button>
              ))}
            </div>
          )}
        ...
        {tools.map((tool) => (
          <button
            ...
            type="button"
            title={tool.label}
            className={`${ib} h-8 w-8 sm:h-9 sm:w-9 rounded-xl text-xs sm:text-sm font-bold`}
          >
            {tool.icon}
          </button>
        ))}
      ...
      {/* LEFT: Object Controller */}
      <div
        className={`absolute inset-y-0 left-0 z-30 border-r border-[#c8c8c8] bg-[#e8e8e8]/98 shadow-xl
        transition-transform duration-300 ease-in-out ${showOC ? 'translate-x-0' : '-translate-x-full'}`}
      ...
      >
        <div className="flex items-center justify-between border-b border-[#c8c8c8] bg-[#dedede] px-2 py-2 text-[11px] font-semibold text-[#111] sm:px-3 sm:text-[13px]">
          <span className="leading-tight">Object Controller</span>
          <button
            type="button"
            onClick={() => setShowOC(false)}
            className={`${ib} h-6 w-6 shrink-0 rounded-md text-xs font-bold`}
          >
            ←
          </button>
        </div>
        ...
        <div className="h-[calc(100%-41px)] overflow-y-auto px-1 py-2 sm:px-2">
          {[
            ...
            <div key={group.key} className={gi > 0 ? 'mt-1 space-y-0.5' : 'space-y-0.5'}>
              ...
              >
                <span className={`${isMobile ? 'text-[9.5px] leading-tight' : 'text-[13px] leading-tight'}`}>
                  {group.key}
                </span>
                <span className="shrink-0 text-[8px] sm:text-[10px]">
                ...
                <div className="ml-1.5 space-y-0.5 border-l border-[#c8c8c8] pl-1.5 sm:ml-3 sm:pl-2.5">
                  {group.items.map((item) => (
                    <button
                      ...
                      className={`block w-full rounded-md px-1 py-1 text-left transition-all sm:px-2.5 sm:py-1.5
                        ${isMobile ? 'text-[9px] leading-tight' : 'text-[12px]'}
                        ${
                          selectedObjectItem === item
                            ? 'bg-[#111] text-white'
                            : 'text-[#555] hover:bg-[#d4d4d4] hover:text-[#111]'
                        }`}
                    >
                      {item}
                    </button>
      ...
      {/* RIGHT / BOTTOM: Object Editor */}
      {selectedObjectId &&
        (isMobile ? (
          <div
            className="absolute bottom-0 left-0 right-0 z-30 flex flex-col
            rounded-t-xl border-t border-[#c8c8c8] bg-[#e8e8e8]/98 shadow-2xl"
            style={{ maxHeight: '50vh' }}
          >
            <div className="flex shrink-0 justify-center pb-1 pt-2">
              <div className="h-[3px] w-8 rounded-full bg-[#c0c0c0]" />
            </div>
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-[#c8c8c8] bg-[#dedede] px-3 py-1.5">
              ...
              <div className="flex items-center gap-1.5">
                <button type="button" onClick={zoomToSelected} className={`${ib} h-6 w-6 rounded-md text-xs`}>
                  ⌖
                </button>
                {!isEditing && (
                  <button onClick={startEditing} className={`${gb} rounded-md px-2 py-1 text-[11px] font-medium`}>
                    Edit
                  </button>
                )}
                <button type="button" onClick={closeEditor} className={`${ib} h-6 w-6 rounded-md text-xs font-bold`}>
                  ↓
                </button>
              </div>
            </div>
              ...
              {(['Details', 'Layers'] as const).map((tab) => (
                <button
                  ...
                  className={`px-4 py-1.5 transition ${
                    activeTab === tab
                      ? 'border-b-2 border-[#111] bg-[#e8e8e8] font-semibold text-[#111]'
                      : 'text-[#555] hover:bg-[#d4d4d4]'
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="flex-1 overflow-y-auto bg-[#e8e8e8]">
              {activeTab === 'Details' ? renderDetails(true) : renderLayers(true)}
            </div>

            {isEditing && (
              <div className="flex shrink-0 items-center justify-end gap-1.5 border-t border-[#c8c8c8] bg-[#dedede] px-2 py-1.5">
                <button onClick={cancelEditing} className={`${gb} rounded-md px-2 py-1 text-[11px] font-medium`}>
                  Cancel
                </button>
                <button onClick={deleteObj} className={`${gb} rounded-md px-2 py-1 text-[11px] font-medium`}>
                  Delete
                </button>
                <div className="relative">
                  <button
                    ...
                    className={`${pb} rounded-md px-2 py-1 text-[11px] font-medium`}
                  >
                    Save ▾
                  </button>
                  {showSaveMenu && (
                    <div className="absolute bottom-full right-0 mb-1 min-w-[140px] overflow-hidden rounded-md border border-[#c8c8c8] bg-[#e8e8e8] shadow-lg">
                      <button
                        ...
                        className="block w-full px-3 py-2 text-left text-[11px] text-[#111] hover:bg-[#d4d4d4]"
                      >
                        Save
                      </button>
                      <button
                        ...
                        className="block w-full border-t border-[#c8c8c8] px-3 py-2 text-left text-[11px] text-[#111] hover:bg-[#d4d4d4]"
                      >
                        Save & Continue
                      </button>
                    </div>
          ...
          <div className="absolute inset-y-0 right-0 z-30 flex w-[320px] flex-col border-l border-[#c8c8c8] bg-[#e8e8e8]/98 shadow-xl">
            <div className="flex items-center justify-between border-b border-[#c8c8c8] bg-[#dedede] px-3 py-2">
              ...
              <button type="button" onClick={closeEditor} className={`${ib} h-7 w-7 rounded-md text-sm font-bold`}>
                →
              </button>
            </div>
            ...
            <div className="flex items-center justify-between gap-2 border-b border-[#c8c8c8] bg-[#e8e8e8] px-3 py-2">
              <div className="truncate text-xs text-[#555]">
                Selected: <span className="font-semibold text-[#111]">{getDisplayLabel()}</span>
              </div>
              <div className="flex items-center gap-2">
                <button type="button" onClick={zoomToSelected} className={`${ib} h-8 w-8 rounded-md text-sm font-bold`}>
                  ⌖
                </button>
                {!isEditing && (
                  <button onClick={startEditing} className={`${gb} rounded-md px-3 py-1.5 text-xs font-medium`}>
                    Edit
                  </button>
                )}
              </div>
            ...
            {isEditing && (
              <div className="flex items-center justify-end gap-2 border-t border-[#c8c8c8] bg-[#dedede] px-2 py-2">
                <button onClick={cancelEditing} className={`${gb} rounded-md px-2.5 py-1.5 text-xs font-medium`}>
                  Cancel
                </button>
                <button onClick={deleteObj} className={`${gb} rounded-md px-2.5 py-1.5 text-xs font-medium`}>
                  Delete
                </button>
                <div className="relative">
                  <button
                    type="button"
                    onClick={() => setShowSaveMenu((p) => !p)}
                    className={`${pb} rounded-md px-2.5 py-1.5 text-xs font-medium`}
                  >
                    Save ▾
                  </button>
                  {showSaveMenu && (
                    <div className="absolute bottom-full right-0 mb-2 min-w-[160px] overflow-hidden rounded-md border border-[#c8c8c8] bg-[#e8e8e8] shadow-lg">
                      <button
                        type="button"
                        onClick={() => saveChanges(false)}
                        className="block w-full px-3 py-2 text-left text-xs text-[#111] hover:bg-[#d4d4d4]"
                      >
                        Save
                      </button>
                      <button
                        type="button"
                        onClick={() => saveChanges(true)}
                        className="block w-full border-t border-[#c8c8c8] px-3 py-2 text-left text-xs text-[#111] hover:bg-[#d4d4d4]"
                      >
                        Save & Continue
                      </button>
                    </div>
      ...
      <div
        className={`absolute z-30 flex flex-col items-center gap-1.5 transition-all duration-300 ${mobileMapControlsOffset}`}
        style={{ left: `${botLeftLeft}px` }}
      >
        <div className="flex flex-col items-center overflow-hidden rounded-2xl border border-[#c8c8c8] bg-[#e8e8e8]/95 shadow-sm">
          <button
            type="button"
            onClick={zoomIn}
            className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center border-b border-[#c8c8c8] text-sm sm:text-base font-bold text-[#111] hover:bg-[#d4d4d4] transition-all"
          >
            +
          </button>
          ...
          <button
            type="button"
            onClick={zoomOut}
            className="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center border-t border-[#c8c8c8] text-sm sm:text-base font-bold text-[#111] hover:bg-[#d4d4d4] transition-all"
          >
            −
          </button>
        </div>
      ...
      <div
        className={`absolute z-30 rounded-xl border border-[#c8c8c8] bg-[#e8e8e8]/95
        px-2 py-1 sm:px-2.5 sm:py-1.5 text-[10px] sm:text-[11px] text-[#555] shadow-sm transition-all duration-300
        ${rightShift} ${mobileLatLonOffset}`}
      >
      ...
      {showBottomPanel && (
        <div
          className={`absolute bottom-0 left-0 right-0 z-40 border-t border-[#c8c8c8] bg-[#e8e8e8]/98 shadow-xl
        ...
        >
          <div className="flex items-center gap-1.5 border-b border-[#c8c8c8] bg-[#dedede] px-2 py-1.5 flex-wrap">
            <div className="mr-auto shrink-0 truncate text-[12px] font-semibold text-[#111]">
              {selectedObjectItem ? `${selectedObjectItem}` : 'Objects'}
            </div>
            ...
            <button
              type="button"
              onClick={() => setAppliedTableFilter(tableFilterInput)}
              className={`${pb} h-7 rounded-md px-2.5 text-[11px] font-semibold`}
            >
              Run
            </button>
            ...
            <button
              ...
              className={`${gb} h-7 rounded-md px-2.5 text-[11px] font-semibold`}
            >
              Download
            </button>
            ...
            <button
              ...
              className={`${ib} h-7 w-7 rounded-md text-xs font-bold`}
            >
              ↓
            </button>
          </div>
              ...
              <thead className="sticky top-0 bg-[#dedede]">
                <tr className="text-left text-[#111]">
                  {['ID', 'Type', 'Status', 'Owner', 'Material', 'Height', 'Municipality', 'Design ID'].map((h) => (
                    <th key={h} className="truncate whitespace-nowrap px-2 py-[3px] font-semibold">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {filteredRows.map((row, idx) => (
                  <tr
                    key={row.key}
                    onClick={row.onClick}
                    className={`h-[23px] leading-none transition-colors ${row.onClick ? 'cursor-pointer' : ''}
                      ${
                        row.selected
                          ? 'border-l-2 border-l-[#111] bg-[#d4d4d4]'
                          : idx % 2 === 0
                          ? 'bg-[#e8e8e8] hover:bg-[#d4d4d4]'
                          : 'bg-[#efefef] hover:bg-[#d4d4d4]'
                      }`}
                  >
