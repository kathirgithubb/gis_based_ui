...
export default function UserPanelPage() {
  ...
  const selectObjectItem = (name: string) => {
    ...
    setAppliedTableFilter(''); setTableFilterInput(''); setShowProject(false);
  ...
  // Desktop-only left panel widths (mobile uses full-width bottom sheet)
  ...
  // Shared project cards renderer (desktop sidebar + mobile sheet)
  const renderAssignedProjects = () => (
    <div className="p-3 space-y-3">
      {ASSIGNED_PROJECTS.length === 0 && (
        <div className="text-center text-[12px] text-[#888] py-8">No projects assigned to you yet.</div>
      )}
        ...
        <div key={i} className="rounded-xl border border-[#c8c8c8] bg-white p-3 shadow-sm">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="min-w-0">
              ...
              <div className="text-[12px] text-[#666] mt-0.5">{p.desc || 'No description'}</div>
            </div>
            <span className="shrink-0 rounded-full bg-[#111] px-2 py-0.5 text-[10px] font-semibold text-white leading-none mt-0.5">{p.status}</span>
          </div>
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-[#f2f2f2] font-sans text-[#111]">
      ...
      {/* ── DESKTOP: Object Controller left panel ── */}
      {!isMobile && (
      ...
      {/* ── DESKTOP: My Projects left panel ── */}
